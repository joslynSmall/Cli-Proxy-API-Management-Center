...
class ApiClient {
  ...
  buildAuthorizedHeaders(headers?: HeadersInit): Headers {
    const resolved = new Headers(headers);
    if (this.managementKey && !resolved.has('Authorization')) {
      resolved.set('Authorization', `Bearer ${this.managementKey}`);
    }
    return resolved;
  }
}
